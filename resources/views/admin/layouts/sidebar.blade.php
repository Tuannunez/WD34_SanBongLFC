...
<aside class="admin-sidebar" id="adminSidebar">
    ...
    <div class="sidebar-menu">
        ...
        <button
            type="button"
            class="sidebar-link w-100 border-0 {{ $bookingOpen ? 'active' : '' }}"
            data-bs-toggle="collapse"
            data-bs-target="#bookingManagementMenu"
            aria-expanded="{{ $bookingOpen ? 'true' : 'false' }}"
            aria-controls="bookingManagementMenu"
        >
            <i class="bi bi-calendar2-check"></i>

            <span class="flex-grow-1 text-start">Đơn đặt sân</span>

            <i class="bi bi-chevron-down small"></i>
        </button>

        <div
            class="collapse {{ $bookingOpen ? 'show' : '' }}"
            id="bookingManagementMenu"
        >
            <a
            ...
            @if(\Illuminate\Support\Facades\Route::has('admin.booking-services.index'))
                <a
                    href="{{ route('admin.booking-services.index') }}"
                    class="sidebar-link ps-5 {{ request()->routeIs('admin.booking-services.*') ? 'active' : '' }}"
                >
                    <i class="bi bi-basket"></i>
                    <span>Dịch vụ theo đơn</span>
                </a>
            @endif

            @if(\Illuminate\Support\Facades\Route::has('admin.booking-details.index'))
                <a
                    href="{{ route('admin.booking-details.index') }}"
                    class="sidebar-link ps-5 {{ request()->routeIs('admin.booking-details.*') ? 'active' : '' }}"
                >
