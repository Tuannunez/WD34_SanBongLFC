    ...
    <div style="max-width:520px;margin:40px auto;padding:24px;background:#fff;border:1px solid #e5e7eb;border-radius:16px;box-shadow:0 10px 30px rgba(15,23,42,0.08);">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;">
            <div>
                ...
                <h1 style="margin:0;font-size:28px;color:#111827;">Đăng ký tài khoản</h1>
            </div>
        </div>
        ...
        <div style="text-align:center;color:#6b7280;font-size:14px;margin-bottom:20px;">Hoặc đăng ký bằng email</div>
        ...
        <form method="POST" action="{{ route('register.store') }}">
            ...
            <div style="margin-bottom:16px;">
                <label for="name" style="display:block;font-weight:600;color:#111827;margin-bottom:6px;">Họ và tên</label>
                <input id="name" name="name" value="{{ old('name') }}" required autofocus style="width:100%;padding:12px;border:1px solid #d1d5db;border-radius:12px;background:#f8fafc;" placeholder="Nhập họ và tên">
                @error('name')<p style="color:#b91c1c;font-size:13px;margin-top:6px;">{{ $message }}</p>@enderror
            </div>
            ...
            <div style="margin-bottom:16px;">
                ...
                @error('email')<p style="color:#b91c1c;font-size:13px;margin-top:6px;">{{ $message }}</p>@enderror
            </div>
            ...
            <div style="margin-bottom:24px;">
                <label for="password_confirmation" style="display:block;font-weight:600;color:#111827;margin-bottom:6px;">Xác nhận mật khẩu</label>
                <input id="password_confirmation" type="password" name="password_confirmation" required style="width:100%;padding:12px;border:1px solid #d1d5db;border-radius:12px;background:#f8fafc;" placeholder="Nhập lại mật khẩu">
                @error('password_confirmation')<p style="color:#b91c1c;font-size:13px;margin-top:6px;">{{ $message }}</p>@enderror
            </div>

            <button type="submit" style="width:100%;background:#16a34a;color:#fff;padding:14px;border-radius:14px;border:none;font-weight:700;">Hoàn tất đăng ký</button>
        </form>

        <div style="text-align:center;color:#6b7280;font-size:13px;margin-top:20px;">Đã có tài khoản? <a href="{{ route('login') }}" style="color:#2563eb;text-decoration:none;">Đăng nhập</a></div>
    </div>
</body>
</html>
